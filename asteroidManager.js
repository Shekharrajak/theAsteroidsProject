    ...
    this.breakAsteroid = function(index, pieces){
        ...
        if(this.asteroids[index].level===1){
            ...
            for(var i=0; i<pieces; i+=1){
                ...
                this.addAsteroid([this.asteroids[index].position.x, this.asteroids[index].position.y], [force.x, force.y], this.asteroids[index].level-1);
            }
            this.removeAsteroid(index);
        }
    };
    
    // This will only detect collisions with round objects. Returns asteroid collided with.
